    ...
    ui(new Ui::Dialog)
{
    ...
    // Connecter un QTimer à une fonction qui sera appelée toutes les 10 secondes
        ...
        int ret = A.connect_arduino("COM8");
           switch (ret) {
           case 0:
...
{
    if (A.getserial()->isOpen() && A.getserial()->bytesAvailable() > 0) {
        QByteArray data = A.read_from_arduino();
        QString keyPressed = QString::fromUtf8(data);

        if (!keyPressed.isEmpty()) {
            qDebug() << "Received data:" << keyPressed;
            ...
            if (keyPressed == "*") {
                ...
                on_verify_clicked();  // Assuming on_verify_clicked is a valid function in your Dialog class
            } else {
                // Insert numeric or character data into the line edit
                ui->lineEdit_id_arduino->insert(keyPressed);
                ui->lineEdit_id_arduino->setFocus();
            }
        }
    }
}
...
void Dialog::on_verify_clicked()
{
    ...
    qDebug() << data;
               ...
               if(f.recherche(data))
               {
                   ...
                   f.affiche_2(data);
                       ui->tableView_facture_2->setModel(f.affiche_2(data));
