...
void MainWindow::add_Gestionbtn()
{
 ...
 if(curr_user.get_role()=="Admin" || curr_user.get_role()=="Employe"){// Create a pushbutton For Gestion_emp
     ...
     GestionPiece_btn->setStyleSheet("QPushButton:hover{background-color: rgb(71,186, 205);"
                              "border: 5px solid black;"
                              "  box-shadow: rgba(72, 135, 202, 0.8) 0 0 90px 33px;"
                              "border-radius:15px;}"
 ...
 if(curr_user.get_role()=="Admin" || curr_user.get_role()=="Finance"){// Create a pushbutton For Gestion_reservation
     ...
     // Set the button's size (optional)
     GestionFinance_btn->setFixedSize(250,150 );

     // Set an icon for the button (optional)
     QIcon icon(":/img/image/Gestion_finance.png");  // Replace with the actual path to your icon

     GestionFinance_btn->setIcon(icon);
     QSize size(250,130);
     GestionFinance_btn->setIconSize(size);
     currentHLayout->addWidget(GestionFinance_btn);
     ...
     GestionFinance_btn->setStyleSheet("QPushButton:hover{background-color: rgb(255, 255, 255);"
                              "border: 5px solid black;"
                              "  box-shadow: rgba(72, 135, 202, 0.8) 0 0 90px 33px;"
                              "border-radius:15px;}"
                               "QPushButton{background-color: rgb(255, 255, 255);"
     ...
     // Create a drop shadow effect
         ...
         GestionFinance_btn->setMouseTracking(true);

            // Install an event filter to handle enterEvent and leaveEvent
            GestionFinance_btn->installEventFilter(this);
 ...
 if(curr_user.get_role()=="Admin" || curr_user.get_role()=="Employe"){// Create a pushbutton For Gestion_reservation
     ...
     connect(GestionVehi_btn,SIGNAL(clicked()),this,SLOT(onVehiculeButtonClicked()));
    }
}
...
void MainWindow::onFinanceButtonClicked()
{
    this->hide();

    // Create the rh window if it doesn't exist
    Dialog  *finance = new Dialog;

    // Connect the destroyed signal to the onRhWindowClosed slot
    connect(finance, &QDialog::finished, this, &MainWindow::onRhWindowClosed);

    finance->show();
    finance->activateWindow();

}

void MainWindow::onVehiculeButtonClicked()
{
    this->hide();
...
bool MainWindow::eventFilter(QObject *obj, QEvent *event) {
    ...
    if (obj == GestionFinance_btn) {
        if (event->type() == QEvent::Enter) {
            ...
            // Mouse left the button
            GestionFinance_btn->setGraphicsEffect(nullptr);
        }
    }
